...
// SOMENTE variável de ambiente — nunca coloque senha no código
const DATABASE_URL = process.env.DATABASE_URL;
if (!DATABASE_URL) {
  console.error("[FATAL] Defina DATABASE_URL no Render (Environment).");
  process.exit(1);
}
...
/** Converte lat/lng em endereço legível (Nominatim / OpenStreetMap, grátis) */
  ...
  try {
    ...
    const r = await fetch(url, {
      headers: {
        ...
        "User-Agent": "RotasGO/7.3 (ride-app; contact@rotasgo.local)",
      },
    });
    if (!r.ok) return null;
    ...
    console.error("[reverseGeocode]", e.message);
    return null;
  }
}

function looksLikeCoordsOnly(label) {
  if (!label) return true;
  const t = String(label).trim();
  // "Minha localização (-16.29, -48.95)" ou só números
  if (/minha localiza/i.test(t)) return true;
  if (/^-?\d+\.?\d*\s*,\s*-?\d+\.?\d*$/.test(t)) return true;
  if (/localiza[cç][aã]o\s*\(/i.test(t)) return true;
  if (/origem no mapa/i.test(t)) return true;
  return false;
}

/** Garante label de endereço; se for coordenada/genérico, faz reverse geocode */
async function resolveAddressLabel(lat, lng, existingLabel) {
  if (existingLabel && !looksLikeCoordsOnly(existingLabel)) {
    return existingLabel;
  }
  const geo = await reverseGeocode(lat, lng);
  if (geo?.label) return geo.label;
  return existingLabel || `${Number(lat).toFixed(5)}, ${Number(lng).toFixed(5)}`;
}
...
app.get("/api/driver/:driverId", async (req, res) => {
  try {
    return json(res, 200, { ok: true, profile: await publicDriver(req.params.driverId) });
  } catch (e) {
    return json(res, 500, { ok: false, error: e.message });
  }
});

/** Endpoint opcional: converter lat/lng → endereço */
app.get("/api/geocode/reverse", async (req, res) => {
  const lat = Number(req.query.lat);
  const lng = Number(req.query.lng);
  if (!Number.isFinite(lat) || !Number.isFinite(lng)) {
    return json(res, 400, { ok: false, error: "lat_lng_invalidos" });
  }
  const geo = await reverseGeocode(lat, lng);
  if (!geo) return json(res, 404, { ok: false, error: "endereco_nao_encontrado" });
  return json(res, 200, { ok: true, ...geo, lat, lng });
});
...
app.post("/api/location/update", async (req, res) => {
  try {
    ...
    const active = await pool.query(
      `SELECT ride_id, client_id, origin_lat, origin_lng
    ...
    for (const ride of active.rows) {
      ...
      const phase = distM <= ARRIVED_RADIUS_M ? "arrived" : "en_route";
      const message =
        phase === "arrived"
...
async function offerNextDriver(rideId) {
  ...
  rt.timer = setTimeout(async () => {
    ...
    await pool.query(
      `UPDATE rides SET offered_to=NULL, status='searching', updated_at=NOW()
...
app.post("/api/ride/request", async (req, res) => {
  try {
    ...
    // Converte coordenadas em endereço legível para o motorista
    const [originLabel, destLabel] = await Promise.all([
      resolveAddressLabel(oLat, oLng, origin.label || null),
      resolveAddressLabel(dLat, dLng, destination.label || null),
    ]);

    const o = { lat: oLat, lng: oLng, label: originLabel };
    const d = { lat: dLat, lng: dLng, label: destLabel };
    ...
    if (!pricing || !Number.isFinite(Number(pricing.total))) {
      ...
      pricing = {
        ...pricing,
        serviceType,
        serviceLabel: getServicePricing(serviceType).label,
      };
    }
    ...
    if (pendingFee > 0) {
      ...
      await pool.query(`UPDATE client_stats SET pending_fee=0 WHERE client_id=$1`, [
        clientId,
      ]);
    }
...
app.post("/api/ride/:rideId/cancel", async (req, res) => {
  try {
    ...
    if (ride.assigned_driver_id) {
      await pool.query(`UPDATE locations SET status='available' WHERE driver_id=$1`, [
    ...
    if (cancelCount > FREE_CANCEL_LIMIT) {
      ...
      await pool.query(`UPDATE client_stats SET pending_fee=$2 WHERE client_id=$1`, [
        ride.client_id,
        pendingFee,
      ]);
    }
    ...
    if (msg.type === "ride_response") {
      try {
        ...
        if (msg.response === "accept") {
          ...
          await pool.query(`UPDATE locations SET status='busy' WHERE driver_id=$1`, [
            msg.driverId,
          ]);
...
initDb()
  .then(() => {
    server.listen(PORT, () => {
      console.log(`[Rotas GO] V7.3 secure env + reverse geocode on port ${PORT}`);
    });
  })
  .catch((err) => {
